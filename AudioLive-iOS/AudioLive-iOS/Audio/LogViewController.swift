...
class LogCell: UITableViewCell {
    ...
    func update(logModel: ARLogModel) {
        ...
        if logModel.status == .normal {
            colorView.backgroundColor = UIColor(hue: 0.0, saturation: 0.0, brightness: 0.0, alpha: 0.3)
            let text = String(format: "%@ %@", userName!, logModel.text!)

            contentLabel.attributedText = changeFontColor(totalString: text, subString: logModel.userName!, font: UIFont.systemFont(ofSize: 14), textColor: UIColor(hexString: "#7BE3FD"))
