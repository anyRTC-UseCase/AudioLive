...
class ARCreateRoomViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        navigationItem.leftBarButtonItem = createBarButtonItem(title: "创建语音房间")
    ...
    @IBAction func createAudioRoom(_ sender: Any) {
        ...
        if Int(roomName?.count ?? 0) > 0 {
            UIAlertController.showActionSheet(in: self, withTitle: "方案选择", message: nil, cancelButtonTitle: "取消", destructiveButtonTitle: nil, otherButtonTitles: ["RTC 实时互动", "客户端推流到CDN", "服务端推流到CDN"], popoverPresentationControllerBlock: nil) { [weak self] _, _, index in
